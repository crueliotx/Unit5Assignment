//Abdullah Ali Unit 5 assignment

#include <iostream>
#include <string>
#include<locale> //for toupper/tolower
#include<iomanip>
using namespace std;

double weeklyPay(double, double,string,int);// function prototype here
...

{
  ...
  string status = ""; 
  string reset = "\x1b[0m";
  string color = "\x1b[" + to_string(35) + ";4m"; //pink and underlined
  string color2 = "\x1b[" + to_string(34) + ";1m"; //blue

  //display the name on screen upper case and colored
  cout << color << endl; //formatting a new line after each.
  for(int i = 0; i < name.length(); i++ )
  {
    cout << toupper(name[i],loc); 
  }
  cout << reset << endl; //line after names
